            ...
            <div class="card-header d-block">
                <div class="row">
                    ...
                    <div class="col-md-6 col-sm-12 text-right">
                        <a href="{{route('admin.create-knowledge-form')}}" class="btn btn-success"> {{__('Add New Knowledge')}}</a>
                    </div>
                </div>
            </div>
            ...
            <div class="card-body">
                <div class="table-responsive style-scroll">
                    <table id="example" class="table table-striped table-bordered miw-500" cellspacing="0" width="100%">
                        ...
                        <tbody>
                            @foreach($knowledges as $key => $data)
                        <tr>
                            <th>{{$key+1}}</th>
                            <th>{{$data->title}}</th>
                            <td>{{$data->status == 1? "Published" : "Not Published"}}</td>
                            <td>
                                <a href="{{route('admin.create-knowledge-form', $data->id)}}" class="btn btn-info  btn-circle">
                                    <i class="material-icons">visibility</i>
                                </a>
                                <a href="{{route('admin.delete-knowledge', $data->id)}}" title="Delete"
